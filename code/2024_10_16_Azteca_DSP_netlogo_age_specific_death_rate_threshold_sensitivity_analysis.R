...
model.mort.sens.analysis.df <- data.frame(matrix(0,ncol=length(df.names)))
colnames(model.mort.sens.analysis.df) <- df.names
trun.thres.vals.vec <- (seq(1,6.3,0.1))
...
par(mfrow=c(2,5),mai=c(0.6,0.6,0.1,0.1),oma=c(0,0,1,01))
library(brms)
for(x in 1:length(pred.diff.vec)){
#x <- 1
...
a.diff.df <- model.df[model.df $pred.diff == pred.diff.vec[x], ]
head(a.diff.df)
plot(log(a.diff.df$age),log(a.diff.df $death.rate.abs),cex=0.01,xlab="",ylab="")
mtext("ln(age class)",side=1,line=2.2)
mtext("ln(death rate)",side=2,line=2.2)
mtext(paste("consumer diffusion:", pred.diff.vec[x]),side=3,line=0.1,cex=0.6)
...
#points(trunc.df$log.age,log(trunc.df$death.rate.abs),col=magma(length(trun.thres.vals.vec))[trunc.looper],cex=0.2)
...
mixed.eff.trunc.model <- lmer(log(death.rate.abs) ~ log.age + (1 |replicate),data= trunc.df)
summary(mixed.eff.trunc.model)
abline(summary(mixed.eff.trunc.model)$coeff[1],summary(mixed.eff.trunc.model)$coeff[2],col=magma(length(trun.thres.vals.vec))[trunc.looper],lwd=1)
...
plot(NA ,xlim=c(1,6.3),ylim=c(-1.5,0.2),xlab="",ylab="")
mtext("ln(resource age) bin",side=1,line=2.2)
mtext("Estimate of age-specific mortality",side=2,line=2.2)
abline(h=0,lty=2)
...
legend(4,-0.4,col=viridis(10),legend=unique(model.mort.sens.analysis.df $pred.diff),lwd=2,cex=0.5,bty="n",title="diffusion rate")
